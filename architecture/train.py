...
class Train(Trainable):
    def _setup(self, config):
        ...
        datasets = ds.open_dataset(
            ...
            CONSTANTS["img_shape"][-1],
            CONSTANTS["is_3d"],
        )
        _, _, train_loader, val_loader = ds.split_dataset(
            datasets, CONSTANTS["batch_size"], CONSTANTS["frac_val"]
        )
        m, o, s, t, v = train_utils.init_training(self.logdir, train_params, CONSTANTS)
    ...
    def _train_iteration(self):
        ...
        for batch_idx, batch_data in enumerate(self.train_loader):
            ...
            if "adversarial" in train_params["reconstructions"]:
                ...
                labels_from_prior, _, _ = discriminator(batch_from_prior.detach())

                loss_dis_data = F.binary_cross_entropy(labels_data, real_labels)
                loss_dis_from_prior = F.binary_cross_entropy(
                    labels_from_prior, fake_labels
                )

                # TODO(nina): add loss_dis_recon
                loss_discriminator = lambda_adv * (loss_dis_data + loss_dis_from_prior)
                ...
                labels_recon, _, _ = discriminator(batch_recon_detached)
                labels_from_prior, _, _ = discriminator(batch_from_prior)

                loss_generator_recon = F.binary_cross_entropy(labels_recon, real_labels)
            ...
            if "kullbackleibler" in train_params["regularizations"]:
                loss_regularization = lambda_regu * losse.kullback_leibler(mu, logvar)
                # Fill gradients on encoder only
                loss_regularization.backward()

            if "kullbackleibler_circle" in train_params["regularizations"]:
                loss_regularization = losse.kullback_leibler_circle(mu, logvar)
                # Fill gradients on encoder only
                loss_regularization.backward()
        ...
        logging.info("====> Epoch: {} Average loss: {:.4f}".format(epoch, average_loss))
            ...
            for batch_idx, batch_data in enumerate(self.val_loader):
                ...
                if "adversarial" in train_params["reconstructions"]:
                    ...
                    loss_dis_data = F.binary_cross_entropy(
                        labels_data, real_labels.float()
                    )
                    loss_dis_from_prior = F.binary_cross_entropy(
                        labels_from_prior, fake_labels.float()
                    )
                    loss_discriminator = loss_dis_data + loss_dis_from_prior
                    ...
                    loss_generator_recon = F.binary_cross_entropy(
                        labels_recon, real_labels.float()
                    )
                ...
                if "kullbackleibler" in train_params["regularizations"]:
                    loss_regularization = losse.kullback_leibler(mu, logvar)
                if "kullbackleibler_circle" in train_params["regularizations"]:
                    loss_regularization = losse.kullback_leibler_circle(mu, logvar)
...
if __name__ == "__main__":
    ...
    if WITH_RAY:
        ...
        search_space = {
            ...
            "lr": hp.loguniform("lr", low=np.log(0.0001), high=np.log(0.01)),
            ...
            "lambda_regu": hp.loguniform(
                "lambda_regu", low=np.log(0.001), high=np.log(0.5)
            ),
            "lambda_adv": hp.loguniform(
                "lambda_adv", low=np.log(0.001), high=np.log(0.5)
            ),
            "batch_size": 8,
            "n_gan_lay": 4,
            "regu_factor": hp.loguniform(
                "regu_factor", low=np.log(0.001), high=np.log(0.5)
            ),
        ...
        hyperopt_search = HyperOptSearch(
            search_space, metric="average_loss", mode="min", max_concurrent=257
        )
