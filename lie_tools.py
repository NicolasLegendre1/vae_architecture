...
def test_algebra_maps():
    ...
    np.testing.assert_allclose(
        matrices_prime.detach().numpy(), matrices.detach().numpy())
...
def test_wigner_d_matrices():
    ...
    for l in range(6):
        ...
        np.testing.assert_allclose(
            matrices @ matrices.transpose(-2, -1), eye, rtol=1E-4, atol=1E-5)
        ...
        for _ in range(100):
            ...
            np.testing.assert_allclose(
                w @ winv, torch.eye(w.shape[1]), rtol=1E-4, atol=1E-5)
...
def test_ref_wigner_d_matrices():
    ...
    for l in range(6):
        for i in range(1000):
            ...
            np.testing.assert_allclose(SO3_coordinates(
                aa, 'EA323', 'MAT'), ra, atol=1E-6, rtol=1E-6)
            np.testing.assert_allclose(SO3_coordinates(
                ab, 'EA323', 'MAT'), rb, atol=1E-6, rtol=1E-6)
            np.testing.assert_allclose(SO3_coordinates(
                ac, 'EA323', 'MAT'), rc, atol=1E-6, rtol=1E-6)
...
def test_s2s2_gram_schmidt():
    ...
    np.testing.assert_allclose(
        dets, torch.ones_like(dets), rtol=1E-6, atol=1E-6)
    np.testing.assert_allclose(
        r.bmm(r.transpose(1, 2)), I, rtol=1E-6, atol=1E-6)


def ref_s2_rotation(x, a, b, c):
    from s2cnn import so3_rotation
    x = so3_rotation(x.view(*x.size(), 1).expand(*
                     x.size(), x.size(-1)), a, b, c)
    return x[..., 0]


def rot_mat_wigner(angles, wig_dim):
    batch_size = angles.shape[0]
    J = Jd[wig_dim].to(DEVICE)
    matrices = torch.Tensor(batch_size, 2*wig_dim+1, 2*wig_dim+1).to(DEVICE)
    # alpha = tens[:, 0]
    # beta = tens[:, 1]
    # gamma = tens[:, 2]
    for i in range(batch_size):
        ...
        beta = angles[i][1]
        gamma = angles[i][2]
        matrices[i] = rot_mat(alpha, beta, gamma, wig_dim, J)
    return matrices
