...
def plot_losdata(filename, ckpt, nrows=4, ncols=18, figsize=(18, 4), cmap='gray'):
...
def show_img_and_recon(output, dataset_path, algo_name='vae', epoch_id=None,
    ...
    if len(dataset.shape) != 4:
        dataset = dataset.reshape(
            dataset.shape[0], 1, dataset.shape[-1], dataset.shape[-1])
        dataset = torch.Tensor(dataset)
        dataset = ds.normalization_linear(dataset)
    print('Dataset shape:', dataset.shape)
...
def plot_cryo(ax, output, path_vae_param, img_path, labels_path,
    ...
    for mu, colored_label in zip(projected_mus, colored_labels):
        ...
        elif label_name == 'theta' or label_name == 'rotation_x':
        ...
        elif label_name == 'quat_x' or label_name == 'quat_y' or label_name == 'quat_z' or label_name == 'quat_w':
            color_id = int(50*(colored_label+1))

        elif label_name == 'vec_x' or label_name == 'vec_y' or label_name == 'vec_z':
            color_id = int(180*colored_label)

        elif label_name == 'rotation':
            color_id = int(180*colored_label/np.pi)

        elif label_name == 'cosrotationw2' or label_name == 'cosrotationx2' or label_name == 'cosrotationy2' or label_name == 'cosrotationz2':
            color_id = int(50*(colored_label+1))
        elif label_name == 'sum':
            color_id = int(colored_label)
...
def hist_labels(labels):
    ...
    if len(labels.shape) == 1:
        ...
        plt.title('rotation_z')

        ax = fig.add_subplot(412)
        ax = ax.hist(labels['rotation_x'], bins=180)
        plt.title('rotation_x')

        ax = fig.add_subplot(413)
        ax = ax.hist(labels['rotation_y'], bins=180)
        plt.title('rotation_y')


def plot_rotation(description_rot, SUB_PATH, OUTPUT):
    colormap_tw = cm.get_cmap('twilight')
    fig, ax = plt.subplots(nrows=4, ncols=1, figsize=(7, 5))
    SUB_DATASET_PATH = SUB_PATH+description_rot['paths'][0]
    SUB_LABELS_PATH = SUB_PATH+description_rot['paths'][1]
    if description_rot['representation'] == 'quaternion':
        im, ax = plot_cryo(ax, OUTPUT, SUB_DATASET_PATH,
                           SUB_LABELS_PATH, label_name='quat_x')
        cb = fig.colorbar(cm.ScalarMappable(cmap=colormap_tw), ax=ax)
        angle = cb.set_label('X')
        yticklabels = cb.ax.set_yticklabels(
            np.arange(start=-180, stop=181, step=72))

        im, ax = plot_cryo(ax, OUTPUT, SUB_DATASET_PATH,
                           SUB_LABELS_PATH, label_name='quat_y')
        cb = fig.colorbar(cm.ScalarMappable(cmap=colormap_tw), ax=ax)
        angle = cb.set_label('Y')
        yticklabels = cb.ax.set_yticklabels(
            np.arange(start=-180, stop=181, step=72))

        im, ax = plot_cryo(ax, OUTPUT, SUB_DATASET_PATH,
                           SUB_LABELS_PATH, label_name='quat_z')
        cb = fig.colorbar(cm.ScalarMappable(cmap=colormap_tw), ax=ax)
        angle = cb.set_label('Z')
        yticklabels = cb.ax.set_yticklabels(
            np.arange(start=-180, stop=181, step=72))

        im, ax = plot_cryo(ax, OUTPUT, SUB_DATASET_PATH,
                           SUB_LABELS_PATH, label_name='quat_w')
        cb = fig.colorbar(cm.ScalarMappable(cmap=colormap_tw), ax=ax)
        angle = cb.set_label('W')
        yticklabels = cb.ax.set_yticklabels(
            np.arange(start=-180, stop=181, step=72))
...
def hist_predict_R_0(Coords):
    fig = plt.figure(figsize=(24, 16))

    ax = fig.add_subplot(411)
    ax = ax.hist(Coords['R_0Euler_x'], bins=180,)
    plt.title('rotation_x')

    ax = fig.add_subplot(412)
    ax = ax.hist(Coords['R_0Euler_y'], bins=180)
    plt.title('rotation_y')

    ax = fig.add_subplot(413)
    ax = ax.hist(Coords['R_0Euler_z'], bins=180)
    plt.title('rotation_z')
...
def BinImagePlot(b, k_bin):
    ...
    image = det.assemble_image_stack(spi_data['intensities'][true_index])[
        :, det_mask][sub_index][:, sub_index]
    k_mask = (np.arange(b.sum()) == k_bin)
    not_k_mask = np.logical_not(k_mask)
    fig, (ax1, ax2) = plt.subplots(nrows=1, ncols=2, figsize=(12, 6))
    ax1.scatter(coordinates[b][not_k_mask, 0], coordinates[b]
                [not_k_mask, 1], color='blue', alpha=0.3)
    ax1.scatter(coordinates[b][k_mask, 0], coordinates[b]
                [k_mask, 1], s=200, color='r', marker='^')
...
def bin_lap_gif(b_idx, n_frames):
    ...
    b = bin_indexes[b_idx]
    len_bin = b.sum()
    frames = np.linspace(0, len_bin, num=n_frames,
                         endpoint=False, dtype='int32')
